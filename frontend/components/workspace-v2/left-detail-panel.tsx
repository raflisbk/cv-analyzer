...
import type { SuggestionCard, ApiSuggestionItem } from "@/lib/types";
...
function SuggestionsTabContent({
  ...
  suggestions.forEach((card, cardIdx) => {
    card.suggestions.forEach((item: ApiSuggestionItem, itemIdx) => {
      allCards.push({
        ...
        afterText: item.afterText ?? item.after_text ?? null,
      });
    });
  });
  ...
  return (
    <div className="flex flex-col gap-2">
      {allCards.map(({
        suggestionId, section, text, explanation,
        priority, originalText, afterText
      }) => {
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {activeDetailTab === "suggestions" ? (
              ...
              <div className="space-y-4 flex flex-col">
                ...
                <div className="rounded-xl border border-[#F5F2D8]/10
                  bg-[#F5F2D8]/[0.02] p-4 text-xs text-[#F5F2D8]/70
                  leading-relaxed mt-2">
                  <p className="font-semibold text-[#F5F2D8]">How is this calculated?</p>
                  <p className="mt-1">
                    Scores are generated by an advanced AI
                    (Hugging Face BGE-M3 Embeddings) representing semantic
                    alignment with industry standards, rather than a rigid
                    rules-based engine.
                  </p>
                </div>
              </div>
