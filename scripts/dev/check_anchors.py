...
async def check():
    async with async_session_maker() as db:
        result = await db.execute(
            select(Job).where(Job.id == "130a3e9b-58cd-4106-97c6-a9f24c491219")
        )
        job = result.scalar_one_or_none()
        if job:
            print(f"Job status: {job.status}")
            print(
                f"Suggestion cards: {len(job.suggestion_cards) if job.suggestion_cards else 0}"
            )
            print(
                f"Suggestion anchors: {len(job.suggestion_anchors) if job.suggestion_anchors else 0}"
            )
